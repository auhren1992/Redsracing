...
(async function () {
  ...
  const permissionDeniedState = document.getElementById(
  ...
  async function loadTeamMembers(isLoadMore = false) {
    try {
      ...
      if (searchTerm) {
        ...
        filteredMembers = newMembers.filter(
          (member) =>
  ...
  function createMemberCard(member) {
    ...
    card.className =
      "card rounded-lg p-6 hover:bg-slate-800/50 transition-colors cursor-pointer";
    ...
    const roleDisplay = member.role ? formatRole(member.role) : "Member";
  ...
  function showMemberDetails(member) {
    ...
    const favoriteCars = member.favoriteCars
      ? member.favoriteCars
          .map(
            (car) =>
              `<span class="bg-slate-700 px-2 py-1 rounded text-sm">${car}</span>`,
          )
          .join(" ")
  ...
  function updateTeamStats() {
    ...
    const active = members.filter((member) => {
      ...
      const lastActive = member.lastActiveAt.toDate
    ...
    async (user, validToken) => {
      ...
      if (user && validToken) {
        ...
        try {
          ...
          isTeamMember =
            claimsResult.success && claimsResult.claims.role === "team-member";
