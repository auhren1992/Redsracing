...
/**
 * Creates a reCAPTCHA assessment to analyze the risk of a user action.
 *
 * This function is called from the client-side to verify a reCAPTCHA token.
 * It uses the reCAPTCHA Enterprise API to create an assessment and returns
 * the risk analysis score.
 *
 * @param {object} data The data passed to the function.
 * @param {string} data.token The reCAPTCHA token generated by the client.
...
exports.createAssessment = onCall(async (request) => {
    // Note: We are not using the cors middleware here because onCall functions
    // handle CORS automatically. If this were a standard onRequest function,
    // we would need to wrap it with cors, e.g., exports.myFunc = onRequest((req, res) => cors(req, res, async () => { ... }))

    const { token, recaptchaAction } = request.data;
    const projectID = process.env.GCLOUD_PROJECT;
    const recaptchaKey = "6Lefz8orAAAAAFKJygPHNO4_3ppt8G4j-bB8Qp5E"; // This should be a secret
    ...
    try {
        ...
        if (!response.tokenProperties.valid) {
            logger.error(`Invalid reCAPTCHA token: ${response.tokenProperties.invalidReason}`);
            throw new HttpsError("unauthenticated", `The reCAPTCHA token is invalid: ${response.tokenProperties.invalidReason}`);
        }

        if (response.tokenProperties.action !== recaptchaAction) {
            logger.error(`Mismatched reCAPTCHA action. Expected: ${recaptchaAction}, Got: ${response.tokenProperties.action}`);
            throw new HttpsError("permission-denied", "The reCAPTCHA action does not match.");
        }

        logger.info(`reCAPTCHA assessment score for action ${recaptchaAction}: ${response.riskAnalysis.score}`);

        return { score: response.riskAnalysis.score };
    } catch (error) {
