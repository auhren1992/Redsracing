  ...
  try {
    ...
    for (const block of scriptBlocks) {
      ...
      for (const raw of objMatches) {
        ...
        } catch (_) { /* ignore bad JSON fragments */ }
      }
    }
...
async function extractSpeedhiveEventsWithPuppeteer(profileUrl) {
  ...
  try {
    ...
    const racesUrl = profileUrl.replace(/\/profile\/.+$/, 'profile/MYLAPS-GA-3a22ae250e154baf8f798908b7e3599e/races');
    await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120 Safari/537.36');
    await page.goto(racesUrl, { waitUntil: 'networkidle2', timeout: 90000 });
    ...
    const rendered = await page.evaluate(() => {
      ...
      for (const a of anchors) {
        ...
        if (!(/result|event|race/i.test(href) || /Jonathan\s+Kirsch/i.test(label))) continue;
        let date = '';
        try {
...
async function scrapeSpeedhiveJon() {
  const profileUrl = SPEEDHIVE_JON_PROFILE_URL;
  const r = await fetch(profileUrl, { method: 'GET', headers: { 'User-Agent': 'Mozilla/5.0' } });
  const html = await r.text();
  let events = extractSpeedhiveEventsFromHtml(html);

  if (!events.length) {
    try {
      events = await extractSpeedhiveEventsWithPuppeteer(profileUrl);
    } catch (e) {
