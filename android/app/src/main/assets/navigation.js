...
(function () {
        ...
        try {
          ...
          if (user) {
            ...
            async function multiSignOut() {
              try {
                const { safeSignOut } = await import('./assets/js/auth-utils.js');
                await safeSignOut();
                return;
              } catch (_) {}
              // Fallback if auth-utils fails to load
              try {
                localStorage.removeItem('rr_auth_uid');
  ...
  /** True for phone-width browser viewports (not the native WebView shell). */
  ...
  function pinHamburgerLeft(nav, btn) {
    if (nav.firstElementChild === btn) return;
  ...
  function trailClockOnHeader(nav) {
    ...
    if (!clock) return;
    const trail = clock.parentElement === nav ? clock : clock.parentElement;
    if (trail) trail.style.marginLeft = 'auto';
  }
