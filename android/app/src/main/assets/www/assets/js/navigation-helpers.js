...
/**
 * Extract base path without hash for validation
 * @param {string} path - Path potentially with hash fragment
...
function isSafeInternalPath(path) {
  ...
  // Extract base path without hash for validation
  const basePath = extractBasePath(normalizedPath);
  
  // Also check if it's an Android WebView asset path
  if (basePath.startsWith('/assets/www/')) {
    const fileOnly = basePath.replace('/assets/www/', '');
    return ALLOWED_PATHS.has(fileOnly) || ALLOWED_PATHS.has('/' + fileOnly);
  }

  // Check against allowlist (with or without hash)
  return (
    ALLOWED_PATHS.has(basePath) ||
    ALLOWED_PATHS.has(basePath.replace(/^\//, "")) ||
